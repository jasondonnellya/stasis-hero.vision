const NotSupported = () => {
    return (
        <div className="text-center">
            <p className="text-lg text-secondary">Oops! Maybe you're on mobile?</p>
            <p>
                Stasis Hero: Vision is not currently supported on smaller screen sizes. Apologies for any inconvenience.
            </p>
            <p>
                This will probably change in the future so keep up to date with any news by following me on Twitter:
                <br /><br></br>
                <a href="https://twitter.com/StasisHeroDev">@StasisHeroDev</a>
            </p>
        </div>
    )
}

export default NotSupported;